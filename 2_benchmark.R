...
modelelineaire=lm(Zonal_Price~daynum+prev_day_price+prev_day2_price + prev_week_price + Min_Price + Max_price + Forecasted_Zonal_Load + sqrzonalload + cubzonalload,na.action=na.omit ,data=prices2011[169:nrow(prices),])
ytest=prices2011lasso$Zonal_Price
ytrain=prices201213lasso$Zonal_Price
xtrain=as.matrix(prices201213lasso[,c('daynum','prev_day_price','prev_day2_price','prev_week_price','Min_Price','Max_price','Forecasted_Zonal_Load','sqrzonalload','cubzonalload')])
xtest=as.matrix(prices2011lasso[,c('daynum','prev_day_price','prev_day2_price','prev_week_price','Min_Price','Max_price','Forecasted_Zonal_Load','sqrzonalload','cubzonalload')])
...
modelelineaire=lm(Zonal_Price~daynum+prev_day_price+prev_day2_price + prev_week_price + Min_Price + Max_price + Forecasted_Zonal_Load + sqrzonalload + cubzonalload,na.action=na.omit ,data=prices[169:nrow(prices),])
...
stargazer(modlin_tidy,type='latex',title="Résultats de la régression linéaire",summary=FALSE)
plot(modelelineaire)
help(stargazer)
...
stargazer(ridge_tidy,type='latex',title='Résultats du Ridge - Lambda optimal',summary=FALSE)
...
ridgesortant=predict(out,type="coefficients",s=bestlam)
ridge_tidy=tidy(ridgesortant)
stargazer(ridge_tidy,type='latex',title='Résultats du Lasso - Lambda optimal',summary=FALSE)
...
ytest=prices2011lasso$Zonal_Price
ytrain=prices201213lasso$Zonal_Price
xtrain=prices201213lasso[,c('daynum','prev_day_price','prev_day2_price','prev_week_price','Min_Price','Max_price','Forecasted_Zonal_Load','sqrzonalload','cubzonalload')]
xtest=prices2011lasso[,c('daynum','prev_day_price','prev_day2_price','prev_week_price','Min_Price','Max_price','Forecasted_Zonal_Load','sqrzonalload','cubzonalload')]
